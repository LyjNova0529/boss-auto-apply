...
(function () {
  ...
  // 设置求职类型（实习/兼职）：只在筛选栏区域找，避免误点岗位卡片
  async function setJobType(typeName) {
    window.scrollTo(0, 0);
    // 等页面加载出岗位卡片
    for (let w = 0; w < 30; w++) {
      if (document.querySelector('li.job-card-box, [class*="job-card"], [class*="job-list"]')) break;
      await sleep(300);
    }
    await sleep(500);

    // 先收集页面上所有含"实习"的元素信息（调试用）
    var debug = [];
    var allEls = document.querySelectorAll('*');
    for (var i = 0; i < allEls.length; i++) {
      var el = allEls[i];
      if (el.children.length > 5) continue;
      if ((el.textContent || '').trim() !== typeName) continue;
      var parent = el.parentElement;
      var grandpa = parent ? parent.parentElement : null;
      debug.push(el.tagName + '.' + (el.className || '').slice(0, 20) + ' p=' + (parent ? parent.tagName + '.' + (parent.className || '').slice(0, 20) : '') + ' gp=' + (grandpa ? grandpa.tagName + '.' + (grandpa.className || '').slice(0, 20) : ''));
    }

    // 排除岗位列表区域，在剩余区域找"实习"
    var jobList = document.querySelector('[class*="job-list"], [class*="search-job-result"]');
    for (var i = 0; i < allEls.length; i++) {
      var el = allEls[i];
      if (el.children.length > 3) continue;
      if ((el.textContent || '').trim() !== typeName) continue;
      // 跳过岗位列表区域内的元素
      if (jobList && (jobList === el || jobList.contains(el))) continue;
      // 跳过岗位卡片内的元素
      var card = el.closest('li[class*="job-card"], [class*="job-card-box"], [class*="job-item"]');
      if (card) continue;
      // 去掉 a 标签的 href 防止跳转
      if (el.tagName === 'A') el.removeAttribute('href');
      var p = el.parentElement;
      if (p && p.tagName === 'A') p.removeAttribute('href');
      try {
    ...
    return { success: false, error: '未找到"' + typeName + '"，发现：' + JSON.stringify(debug.slice(0, 5)) };
  }
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    if (msg.type === 'SET_JOB_TYPE_BOSS') {
      setJobType(msg.typeName || '实习').then(r => sendResponse(r)).catch(e => sendResponse({ success: false, error: e.message }));
      return true;
    }
