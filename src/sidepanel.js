...
const CFG_FIELDS = ['dsKey', 'resumeText', 'keyword', 'city', 'jobType', 'count', 'minSalary', 'blacklist'];
...
function renderReview(screened) {
  ...
  skipped.forEach(j => {
    const badge = j.filteredByRule ? '<span class="badge-rule">规则过滤</span>' : '';
    html += '<div class="job-item skip"><input type="checkbox" disabled data-id="' + esc(j.id) + '">'
      + '<div class="job-main"><div class="job-title">' + esc(j.name) + '</div>'
      + '<div class="job-sub">' + esc(j.company) + ' · ' + esc(j.salary) + '</div>'
      + '<div class="job-reason s">✗ ' + badge + esc(j.reason) + '</div></div></div>';
  });
